...
const STADIUMS = {
  ...
  chinnaswamy: {
    ...
    capacity: 40000,
    zones: ['West Stand', 'East Stand', 'North Block', 'South Block', 'Executive Box', 'Parking Zone']
  }
};

const FAKE_SITUATIONS = [
  { type: 'WARN', msg: "Minor altercation reported near Food Court. Security notified.", icon: '👮' },
  { type: 'INFO', msg: "Celebrity arrival at VIP Gate 1. Crowd surge expected.", icon: '🌟' },
  { type: 'RISKY', msg: "Medical assistance required at North Stand. Clear path for EMS.", icon: '🚑' },
...
function updateSimulation() {
  const states = ['SAFE', 'MODERATE', 'RISKY'];
  ...
  state.venue.density = state.venue.occupancy > 75 ? 'HIGH' : (state.venue.occupancy > 45 ? 'MED' : 'LOW');
...
function syncUI() {
  ...
  if (safetyGrid) {
    safetyGrid.innerHTML = Object.keys(state.zones).map(id => {
      const z = state.zones[id];
  ...
  if (map) { map.style.opacity = '0.8'; setTimeout(() => map.style.opacity = '1', 100); }
  
  // Clock
  updateText('ist-clock', new Date().toLocaleTimeString('en-US', { hour12: false, timeZone: 'Asia/Kolkata' }) + ' IST');
}
